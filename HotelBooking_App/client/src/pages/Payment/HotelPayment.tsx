...
export default function HotelPaymentPage() {
  ...
  useEffect(() => {
    if (!bookingId) return;
    setLoading(true);
    instanceClient.get(`/hotel-booking/${bookingId}`)
      .then(res => {
  ...
  const handleConfirmPayment = async () => {
    ...
    setConfirming(true);
    try {
      await instanceClient.put(`/hotel-booking/confirm-payment/${bookingId}`);
      navigate("/");
    } catch (err) {
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-white">Thanh toán đặt phòng khách sạn</h1>
          ...
          <div className="p-6 space-y-6">
                ...
                <div className="space-y-3 text-sm lg:col-span-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="bg-white p-3 rounded-lg border">
                      ...
                      <p className="font-semibold text-gray-800">{booking.hotel?.hotelName || "N/A"}</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg border">
            ...
            <section className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-800">Thông tin đặt phòng</h2>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-gray-600">Ngày nhận phòng:</p>
                  <p className="font-semibold text-gray-800">{new Date(booking.checkInDate).toLocaleDateString('vi-VN')}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                  <p className="text-gray-600">Ngày trả phòng:</p>
                  <p className="font-semibold text-gray-800">{new Date(booking.checkOutDate).toLocaleDateString('vi-VN')}</p>
                </div>
                <div className="bg-white p-3 rounded-lg border">
                ...
                <div className="bg-white p-3 rounded-lg border">
                  ...
                  <p className="font-semibold text-gray-800">{booking.contactInfo?.fullName || "N/A"}</p>
                </div>
              </div>
            </section>
            ...
            <section className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-6 border-2 border-dashed border-orange-300">
              ...
              <div className="bg-white p-4 rounded-lg border border-orange-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <button
                    className="flex items-center justify-center px-6 py-3 text-white bg-gradient-to-r from-green-600 to-green-700 rounded-lg hover:from-green-700 hover:to-green-800 focus:outline-none focus:ring-4 focus:ring-green-200 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl"
                  ...
                  <button
                    onClick={() => navigate("/")}
                    className="flex items-center justify-center px-6 py-3 text-gray-700 bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg hover:from-gray-200 hover:to-gray-300 focus:outline-none focus:ring-4 focus:ring-gray-200 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl"
                  >
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                    </svg>
