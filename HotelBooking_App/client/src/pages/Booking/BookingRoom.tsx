...
const BookingRoom = () => {
  ...
  if (checkInDate && checkOutDate && !isNaN(checkInDate.getTime()) && !isNaN(checkOutDate.getTime())) {
    ...
    if (numberOfNights < 0) {
      numberOfNights = 0;
      console.warn('Ngày trả phòng phải sau ngày nhận phòng');
    }
  } else {
    console.warn('Ngày nhận/trả phòng không hợp lệ');
  }
  const formattedCheckIn = checkInDate && !isNaN(checkInDate.getTime()) 
    ? dayjs(bookingData?.check_in_date ?? "").add(7, 'hour').format("DD/MM/YYYY [lúc] HH:mm")
    : "Chưa xác định";
  const formattedCheckOut = checkOutDate && !isNaN(checkOutDate.getTime())
    ? dayjs(bookingData?.check_out_date ?? "").add(7, 'hour').format("DD/MM/YYYY [lúc] HH:mm")
    : "Chưa xác định";
      ...
      if (paymentMethod === "bank_transfer") {
        try {
          ...
          if (res.data?.success && res.data?.paymentUrl) {
            ...
            // Chuyển về trang chủ nếu không có paymentUrl
        ...
        // Nếu không phải thanh toán qua VNPay, chuyển về trang chủ
        window.location.href = '/';
      }
    }
  })
      ...
      <div className="flex gap-6 p-6">
        ...
        <div className="w-3/12 bg-white rounded-xl p-3 shadow-md flex flex-col gap-2">
          ...
          <div className="text-xs text-gray-700 mb-1">
            {room?.locationId?.locationName || 'Không có thông tin địa điểm'}
          </div>
