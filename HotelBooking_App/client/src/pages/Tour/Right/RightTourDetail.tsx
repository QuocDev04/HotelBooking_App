...
interface RightTourDetailProps {
    selectedRoom: { nameRoom: string, priceRoom : string} | null
}
const RightTourDetail = ({ selectedRoom }: RightTourDetailProps) => {
    ...
    const total = (adults + children + infants) * pricePerPerson + selectedRoom?.priceRoom;
    return (
        <div className="max-w-[460px] w-full bg-blue-100/90 p-5 max-md:mt-16 border rounded-4xl border-gray-300/70">
            ...
            {selectedRoom ? (
                <>
                    <div><strong>Tên phòng:</strong> {selectedRoom.nameRoom}</div>
                    <div className="text-gray-600 my-3">{selectedRoom.typeRoom}</div>
                    <div className='mb-3'><strong>Giá:</strong> {new Intl.NumberFormat('vi-VN').format(selectedRoom.priceRoom)} ₫</div>
                </>
            ) : (
