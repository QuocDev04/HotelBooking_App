...
interface RightTourDetailProps {
    selectedRoom: { nameRoom: string; priceRoom: string; typeRoom?: string } | null;
}
...
const RightTourDetail = ({ selectedRoom }: RightTourDetailProps) => {
    ...
    const nights = getNights(tours?.duration || ""); // ví dụ: 5
    const roomPrice = Number(selectedRoom?.priceRoom);
    ...
    return (
        <div className="max-w-[460px] w-full bg-blue-100/90 p-5 max-md:mt-16 border rounded-4xl border-gray-300/70">
            ...
            <div className="space-y-2 my-6">
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center rounded-xl border border-gray-200 dark:border-blue-600">
                        <button
                            onClick={() => setAdultsTour(adultsTour > 1 ? adultsTour - 1 : 1)}
                            className="px-2"
                        >
                            -
                        </button>
                        ...
                        />
                        <button
                            onClick={() => setAdultsTour(adultsTour + 1)}
                            className="px-2"
                        >
                            +
                        </button>
                ...
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center rounded-xl border border-gray-200 dark:border-blue-600">
                        <button
                            onClick={() => setChildrenTour(childrenTour > 0 ? childrenTour - 1 : 0)}
                            className="px-2"
                        >
                            -
                        </button>
                        ...
                        />
                        <button
                            onClick={() => setChildrenTour(childrenTour + 1)}
                            className="px-2"
                        >
                            +
                        </button>
            ...
            {selectedRoom ? (
                <>
                    <div>
                        <strong>Tên phòng:</strong> {selectedRoom.nameRoom}
                    </div>
                    <div className="text-gray-600 my-3">{selectedRoom.typeRoom}</div>
                    <div className="mb-3">
                        <strong>Giá / đêm:</strong>{' '}
                        {new Intl.NumberFormat('vi-VN').format(roomPrice)} ₫ x {nights} đêm
                    </div>
                    <div className="mb-3 text-red-600 font-semibold">
                        <strong>Tổng tiền phòng:</strong>{' '}
                        {new Intl.NumberFormat('vi-VN').format(totalRoomPrice)} ₫
                    </div>
                </>
            ) : (
