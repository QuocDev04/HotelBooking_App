...
import { SearchOutlined, EnvironmentOutlined, StarOutlined, WifiOutlined, CalendarOutlined } from '@ant-design/icons';
...
const HotelListEnhanced: React.FC = () => {
  ...
  // Handle range picker change (fallback)
  const handleRangePickerChange = (dates: any) => {
    if (!dates || dates.length === 0) {
      setFilters(prev => ({ ...prev, checkIn: undefined, checkOut: undefined }));
      return;
    }
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <Card style={{ 
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} md={6}>
              <Button
                ...
                type={filters.checkIn && filters.checkOut ? 'primary' : 'default'}
              >
                {filters.checkIn && filters.checkOut 
                  ? `${moment(filters.checkIn).format('DD/MM')} - ${moment(filters.checkOut).format('DD/MM')}`
                  : 'Chọn ngày'
                }
              </Button>
            </Col>
          ...
          {/* Enhanced Date Picker - Collapsible */}
          {datePickerVisible && (
          ...
          {/* Fallback Range Picker for simple usage */}
          {!datePickerVisible && (
            <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
              <Col xs={24} md={8}>
                <RangePicker
                  ...
                  value={filters.checkIn && filters.checkOut ? [moment(filters.checkIn), moment(filters.checkOut)] : null}
                />
              </Col>
              <Col xs={12} md={4}>
                <Input
        ...
        {(filters.checkIn || filters.checkOut || filters.city || filters.guests) && (
          <Card size="small" style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontWeight: '600', color: '#666' }}>Bộ lọc đang áp dụng:</span>
              {filters.checkIn && filters.checkOut && (
                <Tag closable onClose={() => handleFilterChange('checkIn', undefined) || handleFilterChange('checkOut', undefined)}>
                  📅 {moment(filters.checkIn).format('DD/MM')} - {moment(filters.checkOut).format('DD/MM')}
                </Tag>
              )}
              {filters.city && (
                <Tag closable onClose={() => handleFilterChange('city', undefined)}>
                  📍 {filters.city}
                </Tag>
              )}
              {filters.guests && (
                <Tag closable onClose={() => handleFilterChange('guests', undefined)}>
                  👥 {filters.guests} khách
                </Tag>
              )}
              {filters.starRating && (
                <Tag closable onClose={() => handleFilterChange('starRating', undefined)}>
                  ⭐ {filters.starRating} sao
                </Tag>
              )}
            </div>
          </Card>
...
export default HotelListEnhanced;
