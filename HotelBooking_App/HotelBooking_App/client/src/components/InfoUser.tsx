...
const InfoUser = () => {
    ...
    const filteredbookings = selectedStatus === "all"
        ...
        : selectedStatus === "deposit_paid"
            ? bookings.filter((bill: any) => bill.payment_status === 'deposit_paid' || (bill.isDeposit && !bill.isFullyPaid))
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {bookings.filter((bill: any) => bill.payment_status === 'deposit_paid' || (bill.isDeposit && !bill.isFullyPaid)).length}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${selectedStatus === "deposit_paid"
                                    ? "bg-blue-600 text-white shadow-lg"
                                    : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                                    }`}
                            >
                                    ...
                                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                                        ...
                                        <div className="flex flex-col items-end space-y-4">
                                            ...
                                            <div className="text-right">
                                                ...
                                                {/* Hiển thị thông tin đặt cọc trong danh sách */}
                                                {bill.isDeposit && !bill.isFullyPaid && (
                                                    <div className="mt-1 text-sm">
                                                        <span className="text-blue-600 font-medium">Đã đặt cọc: {bill.depositAmount.toLocaleString()}₫</span>
                                                        <br />
                                                        <span className="text-red-600 font-medium">Còn lại: {(bill.totalPriceTour - bill.depositAmount).toLocaleString()}₫</span>
                                                    </div>
                                                )}
                                                {bill.isDeposit && bill.isFullyPaid && (
                                                    <div className="mt-1 text-sm">
    ...
    <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full border border-gray-200 animate-fadeIn max-h-[90vh] overflow-y-auto">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4 flex flex-col gap-2">
          <div className="font-semibold text-lg text-gray-900">{bookingToCancel.slotId.tour.nameTour}</div>
          <div className="flex flex-wrap gap-4 text-sm text-gray-600">
            <div>
              <span className="font-medium">Ngày khởi hành:</span> {new Date(bookingToCancel.slotId.dateTour).toLocaleDateString('vi-VN')}
            </div>
            <div>
              <span className="font-medium">Tổng tiền:</span> <span className="text-red-600 font-bold">{bookingToCancel.totalPriceTour.toLocaleString()}₫</span>
            </div>
            <div>
              <span className="font-medium">Trạng thái:</span> <span className={`font-medium ${getStatusColor(bookingToCancel.payment_status)}`}>{getStatusText(bookingToCancel.payment_status)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <p><span className="font-medium">Hủy trong vòng 3 ngày:</span> Không hoàn tiền</p>
            </div>
          </div>
