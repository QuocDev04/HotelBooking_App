...
export const createBookingTour = async (req, res) => {
    ...
    try {
        ...
        if (itemRoom && itemRoom.length > 0) {
            const roomIds = itemRoom.map(item => item.roomId);
            const rooms = await Promise.all(roomIds.map(id => RoomModel.findById(id).session(session)));

            for (let i = 0; i < rooms.length; i++) {
                const room = rooms[i];
                if (!room) {
                    await session.abortTransaction();
                    return res.status(404).json({ message: `Phòng ID ${roomIds[i]} không tồn tại.` });
                }
                
                // Kiểm tra xem phòng có đang được đặt trong khoảng thời gian của tour không
                const isRoomAvailable = !room.availabilitySchedule.some(schedule => 
                    schedule.isBooked && 
                    ((startTime >= schedule.startDate && startTime <= schedule.endDate) ||
                     (endTime >= schedule.startDate && endTime <= schedule.endDate) ||
                     (startTime <= schedule.startDate && endTime >= schedule.endDate))
                );
                
                if (!isRoomAvailable || room.statusRoom !== 'available') {
                    await session.abortTransaction();
                    return res.status(400).json({ message: `Phòng ${room.nameRoom} đã được đặt trong thời gian này.` });
                }
        ...
        // Thêm phòng vào lịch đặt phòng
        if (itemRoom && itemRoom.length > 0) {
            const roomIds = itemRoom.map(item => item.roomId);
            await Promise.all(roomIds.map(async (roomId) => {
                const room = await RoomModel.findById(roomId).session(session);
                if (room) {
                    room.availabilitySchedule.push({
                        startDate: startTime,
                        endDate: endTime,
                        isBooked: true,
                        bookedForTourId: tourId,
