...
const EditTour = () => {
    ...
    useEffect(() => {
        if (data?.data?.tour?.imageTour) {
            setFileList(
                data?.data?.tour?.imageTour.map((url: string, index: number) => ({
                    uid: index.toString(),
        ...
        if (data?.data?.tour.descriptionTour) {
            setValue(data.data.tour.descriptionTour);
        }

        // 👇 Cập nhật form values cho các trường khác
        if (data?.data?.tour) {
            form.setFieldsValue(data.data.tour);
        }
    }, [data?.data]);
