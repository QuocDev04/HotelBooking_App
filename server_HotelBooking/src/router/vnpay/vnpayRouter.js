    ...
    try {
        ...
        if (responseCode === '00') {
            if (isHotelBooking) {
                ...
                updatedBooking = await TourBooking.findByIdAndUpdate(
                    bookingId,
                    ...
                    populate: {
                        path: 'tour',
                        // 1. Lấy thêm trường itemTransport
                        select: 'nameTour duration itemTransport', 
                        populate: {
                            path: 'itemTransport.TransportId',
                            model: 'Transport', // Đảm bảo tên Model là 'Transport'
                            select: 'transportName' // Lấy tên phương tiện
                        }
                    }
                });
            }
            ...
            if (updatedBooking.email) {
                try {
                    ...
                    if (isHotelBooking) {
                        ...
                        const startDate = updatedBooking.slotId?.dateTour ? new Date(updatedBooking.slotId.dateTour) : null;
                        const durationString = updatedBooking.slotId?.tour?.duration || "1 ngày";
                        ...
                        if (startDate) {
                            tourDateStr = startDate.toLocaleDateString('vi-VN');
                            const returnDate = new Date(startDate);
                            returnDate.setDate(startDate.getDate() + durationDays - 1);
                            returnDateStr = returnDate.toLocaleDateString('vi-VN');
                        }
                        
                        // ========== [START] LẤY THÔNG TIN PHƯƠNG TIỆN ==========
                        let transportText = 'Chưa xác định';
                        const transports = updatedBooking.slotId?.tour?.itemTransport;
                        if (transports && transports.length > 0) {
                            // Lấy tên từ các object phương tiện đã được populate
                            transportText = transports
                                .map(item => item.TransportId?.transportName)
                                .filter(Boolean) // Loại bỏ các giá trị null/undefined
                                .join(', ');
                        }
                        // ========== [END] LẤY THÔNG TIN PHƯƠNG TIỆN ==========

                        // ========== CẬP NHẬT EMAIL TEMPLATE ==========
                        const totalPriceVN = (updatedBooking.totalPriceTour || 0).toLocaleString('vi-VN');
                        emailHtml = `
                            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; padding: 20px; border-radius: 8px;">
                                ...
                                <p>Cảm ơn bạn đã đặt và thanh toán thành công cho chuyến đi của mình. Dưới đây là thông tin chi tiết:</p>
                                
                                <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
                                    <h3 style="border-bottom: 2px solid #28a745; padding-bottom: 5px;">Thông tin chuyến đi</h3>
                                    <ul style="list-style: none; padding: 0;">
                                        <li style="margin-bottom: 10px;"><strong>Mã đặt chỗ:</strong> ${bookingId}</li>
                                        <li style="margin-bottom: 10px;"><strong>Tên tour:</strong> ${tourName}</li>
                                        <li style="margin-bottom: 10px;"><strong>Thời gian:</strong> ${durationString}</li>
                                        <li style="margin-bottom: 10px;"><strong>Ngày khởi hành:</strong> ${tourDateStr}</li>
                                        <li style="margin-bottom: 10px;"><strong>Ngày về (dự kiến):</strong> ${returnDateStr}</li>
                                        <li style="margin-bottom: 10px;"><strong>Phương tiện:</strong> ${transportText}</li>
                                    </ul>
                                </div>

                                <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
                                    <h3 style="border-bottom: 2px solid #28a745; padding-bottom: 5px;">Thông tin thanh toán</h3>
                                    <ul style="list-style: none; padding: 0;">
                                        <li style="margin-bottom: 10px;"><strong>Tổng số khách:</strong> 
                                            ${(updatedBooking.adultsTour || 0) + (updatedBooking.childrenTour || 0) + (updatedBooking.toddlerTour || 0) + (updatedBooking.infantTour || 0)} người
                                            (NL: ${updatedBooking.adultsTour || 0}, TE: ${updatedBooking.childrenTour || 0}, TN: ${updatedBooking.toddlerTour || 0}, EB: ${updatedBooking.infantTour || 0})
                                        </li>
                                        <li style="margin-bottom: 10px;"><strong>Tổng giá:</strong> ${totalPriceVN} VNĐ</li>
                                        <li style="margin-bottom: 10px;"><strong>Hình thức:</strong> Thanh toán đầy đủ</li>
                                    </ul>
                                </div>
                                
                                <p>Chúc bạn có một chuyến đi vui vẻ và ý nghĩa!</p>
                                <p>Nếu có bất kỳ thắc mắc nào, vui lòng liên hệ với chúng tôi qua email: <strong>support@example.com</strong></p>
                            </div>
                        `;
                    }
                    await sendMail({
