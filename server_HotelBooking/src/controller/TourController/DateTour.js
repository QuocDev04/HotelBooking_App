...
const assignEmployeeToDateSlot = async (req, res) => {
    try {
        ...
        // employeeId có thể là một ID hợp lệ, hoặc null/undefined/"" để hủy phân công
        const { employeeId } = req.body;

        // --- BƯỚC 1: TÌM SLOT TOUR ---
        ...
        // --- BƯỚC 2: XỬ LÝ LOGIC DỰA TRÊN employeeId ---

        // ----- TRƯỜNG HỢP 1: PHÂN CÔNG MỚI HOẶC THAY ĐỔI PHÂN CÔNG -----
        if (employeeId) {
            // Kiểm tra xem có đang phân công lại cho chính nhân viên đã được phân công không
            if (dateSlot.assignedEmployee && dateSlot.assignedEmployee.toString() === employeeId) {
                return res.status(200).json({
                    success: true,
                    message: 'Nhân viên này đã được phân công cho slot.'
                });
            }
            ...
            if (conflictingSlots.length > 0) {
                return res.status(400).json({
                    success: false,
                    message: 'Nhân viên đã được phân công cho tour khác trong ngày này',
            ...
            await dateSlot.save();
            await dateSlot.populate('assignedEmployee', 'full_name email position');
        ...
        // ----- TRƯỜNG HỢP 2: HỦY PHÂN CÔNG -----
        else {
            // Kiểm tra xem có nhân viên nào đang được phân công để hủy không
            if (!dateSlot.assignedEmployee) {
                // "Không thì thôi" -> Trả về thành công vì đã ở trạng thái mong muốn
                return res.status(200).json({
                    success: true,
                    message: 'Slot này không có nhân viên nào được phân công.'
                });
            }

            // Hủy phân công
            dateSlot.assignedEmployee = null;
            await dateSlot.save();

            return res.status(200).json({
                success: true,
                message: 'Đã hủy phân công HDV thành công',
                data: dateSlot
            });
        }
    } catch (error) {
